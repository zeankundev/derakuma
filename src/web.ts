// Browser version of Derakuma, a Fontobene parser.
...
const FontLoadMethod = {
    FETCH: 'fetch',
} as const;
type FontLoadMethod = (typeof FontLoadMethod)[keyof typeof FontLoadMethod];
    ...
    private async fetchBeneFile(url: string, method: FontLoadMethod | string): Promise<string> {
        ...
        if (hasFetch && method === FontLoadMethod.FETCH) {
            try {
                ...
                throw new Error(`Woopsies! Failed to fetch ${url}! ${response.status} ${response.statusText}`);
            } catch (error) {
                throw error as Error;
            }
        }

        // The "file" load method needs Node's fs/promises and isn't available
        // in the browser build — use src/index.ts (Node build) for that.
        throw new Error(`Unsupported method or missing environment capabilities for ${url}. The browser build of derakuma only supports FontLoadMethod.FETCH.`);
    }
...
// No `export` statements in this file on purpose: it's meant to be dropped
// straight into a <script> tag, not consumed as an ES/CJS module. Everything
// gets attached to the global scope instead, namespaced under `Derakuma`
// (with `DerakumaParser` also exposed directly for convenience).
(function (global: any) {
    global.Derakuma = { DerakumaParser: DerakumaParser, FontLoadMethod: FontLoadMethod };
    global.DerakumaParser = DerakumaParser;
